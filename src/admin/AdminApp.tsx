...
import { PatientReportsTab } from '../components/admin/PatientReportsTab';
import {
  DateRangePreset,
  calculateDateWindow,
...
export const AdminApp: React.FC = () => {
  ...
  // Active Admin Tab
  const [activeTab, setActiveTab] = useState<'appointments' | 'doctors' | 'reviews' | 'analytics' | 'reports'>('appointments');
  ...
  const fetchAllAdminData = async () => {
    ...
    // Patients (sorted A-Z alphabetically)
    try {
      const patRef = collection(db, 'patients');
      const q = query(patRef, orderBy('name', 'asc'));
      let patSnap;
      try {
        patSnap = await getDocs(q);
      } catch (firestoreSortErr) {
        patSnap = await getDocs(patRef);
      }

      const patList: Patient[] = [];
      patSnap.forEach((p) => patList.push({ ...p.data(), uid: p.id } as Patient));

      // Client-side A-Z sort fallback
      patList.sort((a, b) => {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 mt-6 space-y-6">
        ...
        <div className="bg-white p-1.5 rounded-2xl shadow-sm border border-emerald-900/10 flex flex-wrap gap-1">
          ...
          <button
            ...
            className={`flex-1 py-2.5 px-4 rounded-xl text-xs font-bold transition-colors cursor-pointer ${
              activeTab === 'reports' ? 'bg-[#0B6B4E] text-white' : 'text-emerald-900 hover:bg-[#F5F1E8]'
            }`}
          >
            Patient Reports
          </button>
        ...
        {activeTab === 'reports' && (
          <PatientReportsTab patientsList={patients} showToast={showToast} />
        )}
